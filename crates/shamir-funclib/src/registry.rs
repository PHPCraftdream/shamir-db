...
//!
//! A scalar function is a pure `fn(&[QueryValue]) -> ScalarResult`. The
...
pub type ScalarResult = Result<QueryValue, ScalarError>;

/// A scalar function: pure, `Send + Sync`, dispatched by name.
pub type ScalarFn = Arc<dyn Fn(&[QueryValue]) -> ScalarResult + Send + Sync>;
...
impl FnEntry {
    ...
    pub fn pure(
        f: impl Fn(&[QueryValue]) -> ScalarResult + Send + Sync + 'static,
        min_args: usize,
...
// the numeric QueryValue variants (Int / Dec / F64 / Bool-as-0/1 where noted)
// so a category fn does not re-implement coercion.
// ===========================================================================

/// Fetch the `i`-th argument or `ScalarError("missing_arg")`.
fn at(args: &[QueryValue], i: usize) -> Result<&QueryValue, ScalarError> {
...
pub fn arg_i64(args: &[QueryValue], i: usize) -> Result<i64, ScalarError> {
    match at(args, i)? {
        QueryValue::Int(n) => Ok(*n),
        QueryValue::Bool(b) => Ok(*b as i64),
        QueryValue::Dec(d) => {
            if d.fract().is_zero() {
...
pub fn arg_str(args: &[QueryValue], i: usize) -> Result<&str, ScalarError> {
    match at(args, i)? {
...
pub fn arg_f64(args: &[QueryValue], i: usize) -> Result<f64, ScalarError> {
    match at(args, i)? {
        QueryValue::F64(f) => Ok(*f),
        QueryValue::Int(n) => Ok(*n as f64),
        QueryValue::Bool(b) => Ok(if *b { 1.0 } else { 0.0 }),
        QueryValue::Dec(d) => d.to_f64().ok_or_else(|| ScalarError::new("out_of_range")),
        _ => Err(ScalarError::new("type_mismatch")),
    }
}

/// Extract a [`Decimal`]. Accepts `Dec`, `Int`, `Bool` (0/1), and finite `F64`.
pub fn arg_dec(args: &[QueryValue], i: usize) -> Result<Decimal, ScalarError> {
    match at(args, i)? {
...
pub fn arg_bool(args: &[QueryValue], i: usize) -> Result<bool, ScalarError> {
    match at(args, i)? {
...
/// Extract a `&[QueryValue]` from a `List` argument.
pub fn arg_list(args: &[QueryValue], i: usize) -> Result<&[QueryValue], ScalarError> {
    match at(args, i)? {
        QueryValue::List(l) => Ok(l.as_slice()),
        _ => Err(ScalarError::new("type_mismatch")),
    }
}

/// Extract a `&[u8]` from a `Bin` argument.
pub fn arg_bytes(args: &[QueryValue], i: usize) -> Result<&[u8], ScalarError> {
    match at(args, i)? {
...
/// Construct an `Int`.
pub fn v_int(n: i64) -> QueryValue {
    QueryValue::Int(n)
}

/// Construct a `Str`.
pub fn v_str(s: String) -> QueryValue {
    QueryValue::Str(s)
}

/// Construct a `Bool`.
pub fn v_bool(b: bool) -> QueryValue {
    QueryValue::Bool(b)
}
...
/// Construct a `Dec`.
pub fn v_dec(d: Decimal) -> QueryValue {
    QueryValue::Dec(d)
}

/// Construct a `List`.
pub fn v_list(items: Vec<QueryValue>) -> QueryValue {
    QueryValue::List(items)
}

/// Construct a `Bin`.
pub fn v_bytes(b: Vec<u8>) -> QueryValue {
    QueryValue::Bin(b)
}
