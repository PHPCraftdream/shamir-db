//! Local-IPC (Unix domain socket / Windows Named Pipe) transport
//! end-to-end test.
//!
//! Regression coverage for `AcceptPath`-style IPC wiring in
//! `server_launcher.rs` (`ListenerKind::Unix` early-return branch +
//! `accept_loop_ipc`) and `shamir-transport-ipc`'s `IpcListener`. Boots a
//! server with a SINGLE `kind: unix` listener, connects with a bare
//! `UnixStream` (Unix) / Named Pipe client (Windows) — no TLS at all —
//! runs the full SCRAM-Argon2id handshake with `transport_kind = 0x03`,
//! `binding_mode = 0x00` / zeroed channel binding, and executes one batch
//! against the database. Mirrors `plain_tcp_e2e.rs`.
...
// envelopes — same shapes as `mvp_e2e.rs` / `plain_tcp_e2e.rs`.
...
async fn ipc_accept_loop_scram_batch() {
    ...
    // -----------------------------------------------------------------
    // Client side: bare IPC connection, no TLS handshake at all.
    // -----------------------------------------------------------------
    let username = NormalizedUsername::from_raw("admin").expect("username");
    let stream = connect_ipc(&server_addr).await;
    ...
    // `TransportKind::Unix` + `BindingMode::None` — no `.tls_exporter(...)`
    // call, so the builder keeps its zeroed default (matches server-side
    // `[0u8; 32]` for the IPC accept loop).
    ...
    assert!(
        ...
        "server must issue a resumption ticket in auth_ok even on the unix transport",
    );
    ...
    // -----------------------------------------------------------------
    // Post-handshake: one real batch round-trip over the IPC channel.
    // -----------------------------------------------------------------
    ...
    match resp {
        DbResponse::Batch { response } => {
            assert!(
                ...
                "create_db ok over unix transport"
            );
        }
