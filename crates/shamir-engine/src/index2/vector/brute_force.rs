...
//
// Hot path: brute-force search calls `dot_product` / `l2_squared` once
// per stored vector. For typical 128-d f32 vectors that's a clean
// vectorizable inner loop. On x86_64 we runtime-detect AVX2 once and
// dispatch to an AVX2+FMA kernel (8 lanes, fused multiply-add) for a
// repeatable multiple over the autovec scalar path. Fallback is the
// scalar kernel written in a chunked form the compiler also reliably
// vectorizes (no per-iteration bounds checks, multiple accumulators).
//
