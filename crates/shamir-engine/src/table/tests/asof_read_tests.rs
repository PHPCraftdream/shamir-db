...
async fn asof_version_returns_state_at_that_version() {
    ...
    let n2 = res2.records[0].get_value_i64("n").unwrap();
...
async fn asof_excludes_records_not_yet_created() {
    ...
    let name = result.records[0].get_value_str("name").unwrap_or_default();
    assert_eq!(
        name, "alice",
...
async fn asof_timestamp_resolves_or_errors() {
    ...
    let name = res.records[0].get_value_str("name").unwrap_or_default();
...
async fn latest_and_history_unchanged() {
    ...
    let n = res_latest.records[0].get_value_i64("n").unwrap();
    assert_eq!(n, 3, "Latest must return current value n=3");
    assert!(
        res_latest.records[0].get_value_owned("_version").is_none(),
    ...
    let versions: Vec<u64> = res_hist
        .records
        .iter()
        .filter_map(|r| r.get_value_u64("_version"))
        .collect();
