//! P0-2 (#958) — legacy (regular + unique) ops-plan re-derivation + TOCTOU +
//! retired-ops cleanup tests.
//!
//! Mirrors `f50_index_lifecycle_spike_tests.rs`'s red→green proof pattern for
//! the legacy `IndexManager` family (the 2a/2b/2c sub-bugs):
//!
//! - **2a regular/unique**: a tx that STAGES its insert BEFORE a legacy index
...
async fn p02_regular_insert_before_create_posting_present() {
    ...
    // Step 3: COMMIT — the rederive MUST fire (legacy gen advanced).
...
async fn p02_unique_concurrent_duplicate_one_wins() {
    ...
    // T1 commits FIRST: rederive fires (legacy gen advanced), records a
