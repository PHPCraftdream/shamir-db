//! Unit tests for `apply_transforms` (③.2b transform framework).
//!
...
fn auto_now_overwrites_existing_value() {
    ...
    apply_transforms(&mut rec, &transforms, builtin_scalars(), NOW_NS);
...
fn empty_transforms_is_noop() {
    ...
    apply_transforms(&mut rec, &[], builtin_scalars(), NOW_NS);
