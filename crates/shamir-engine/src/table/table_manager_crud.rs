...
impl TableManager {
    ...
    /// Batched update: writes all changed records in a single MVCC
    /// transaction (one fsync instead of N). Each tuple is
    /// `(record_id, old_value, new_value)` — caller has already
    /// computed the merge and filtered unchanged rows.
    ///
    /// Returns the maximum MVCC version assigned across the batch.
    /// Unique-index validation is batched: intra-batch duplicates are
    /// detected in one pass before any writes.
    pub(crate) async fn update_many_returning_version(
        &self,
        pairs: &[(RecordId, &InnerValue, &InnerValue)],
    ) -> DbResult<u64> {
        if pairs.is_empty() {
            return Ok(0);
        }

        // 1. Batched unique-index validation.
        if self.index_manager.has_unique_indexes() {
            let _guard = self.unique_write_lock.lock().await;
            ...
            // Intra-batch collision detection: two rows in the same
            // batch must not claim the same unique key.
            let mut batch_seen: TFxSet<(u64, Vec<u8>)> = TFxSet::default();
                        ...
                        crate::index::index_keys::extract_index_leaves(new_val, &def.paths)
                    {
                        ...
                        if !batch_seen.insert((def.name_interned, key)) {
                            return Err(DbError::DuplicateKey(format!(
                                "Unique index '{}' violated within update batch (row {} duplicates an earlier row)",
                                def.name_interned, i
                            )));
                        }
                    }
        ...
        // 2. Batched data-store write via MVCC (one fsync).
        let batch_version: u64 = if let Some(mvcc) = &self.mvcc_store {
            let items = pairs
                .iter()
                .map(|(id, _old, new_val)| {
                    let bytes = new_val.to_bytes().map_err(|e| {
                        DbError::Codec(format!("Failed to serialize InnerValue: {}", e))
                    })?;
                    Ok((id.to_bytes(), bytes))
                })
                .collect::<DbResult<Vec<_>>>()?;
            mvcc.set_versioned_many(items).await?
        } else {
            // Non-MVCC fallback: per-record set (no batched API on raw store for updates).
            for (id, _old, new_val) in pairs {
                let bytes = new_val.to_bytes().map_err(|e| {
                    DbError::Codec(format!("Failed to serialize InnerValue: {}", e))
                })?;
                self.table.data_store().set(id.to_bytes(), bytes).await?;
