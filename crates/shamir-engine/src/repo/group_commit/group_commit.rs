...
impl GroupCommit {
    ...
    /// Test-only: whether a leader task is currently running. A caller's
    /// `run()` resolving only means the leader SENT that caller's reply —
    /// the leader may not yet have re-locked `state` to release
    /// `leader_busy` (that happens strictly after the batch send, in the
    /// same critical section as the empty-waiters check). A test that wants
    /// to deterministically force the NEXT `run()` call to elect a fresh
    /// leader (rather than possibly racing into the outgoing leader's
    /// queue) must poll this down to `false` first.
    #[cfg(test)]
    pub async fn is_leader_busy(&self) -> bool {
        self.state.lock().await.leader_busy
    }
}
    ...
    Fut: Future<Output = DbResult<()>>,
{
    loop {
        ...
        // Panic-safety (audit group 10 / #1078-adjacent): a panic inside
        // `flush()` must not strand `leader_busy = true` forever — every
        // later `run()` caller would push a oneshot and park with no leader
        // ever elected again (a durability-flush DoS identical in shape to
        // the cancellation hazard fixed above). `catch_unwind` converts a
        // panic into an `Err` so the waiter-notify + `leader_busy = false`
        // release below always runs, exactly like an ordinary `flush()`
        // error. The wrapping `async { flush().await }` block ensures both
        // invoking the closure AND polling the resulting future are covered
        // by the same `catch_unwind`.
        let res: DbResult<()> = match std::panic::AssertUnwindSafe(async { flush().await })
            .catch_unwind()
            .await
        {
            Ok(inner) => inner,
            Err(payload) => {
                ...
                log::error!("group-commit flush() panicked: {msg}");
                Err(DbError::Internal(format!(
                    "group-commit flush panicked: {msg}"
                )))
            }
        };
