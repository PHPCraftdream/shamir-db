...
/// Owned sort key for QueryValue fields. Unlike `SortKey<'a>` this does
/// not borrow from the source records, because `Dec`/`Big` require an
/// owned `String` (the `to_string()` canonical form). Comparison
/// semantics are identical to `compare_sort_keys`.
#[derive(Clone)]
enum QvSortKey {
    Null,
    Bool(bool),
...
impl QvSortKey {
    ...
    /// Mirrors `SortKey::from_json` after the lossy
...
/// Compare two pre-resolved QvSortKey vectors — identical semantics to
/// `compare_preresolved`.
...
/// Identical semantics to `compare_sort_keys`.
#[inline]
