...
1. **Единый формат** — все запросы через `BatchRequest` (MessagePack на проводе, `QueryValue` в памяти)
...
Операция `BatchOp` встраивается напрямую (через `#[serde(flatten)]`). Тип операции определяется автоматически по уникальному ключу `QueryValue`-объекта.
...
`BatchOp` использует **явный key-based dispatch** (не `#[serde(untagged)]`). Десериализация проверяет наличие уникального ключа в `QueryValue`-объекте:

| BatchOp Variant | Ключ | Описание |
|-----------------|-----------|----------|
...
Тип операции определяется по уникальному ключу в `QueryValue`-объекте (ручная десериализация, не `#[serde(untagged)]`):
...
| `float` | `3.14`, `-0.5` |
| `string` | `"hello"` |
| `binary` | `[1, 2, 3]` (байты; на проводе — MessagePack `bin`) |
| `array` | `[1, 2, 3]` |
