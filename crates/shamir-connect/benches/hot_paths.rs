//! Hot-path benchmarks for `shamir-connect`.
//!
//! Run: `cargo bench -p shamir-connect --bench hot_paths`
//!
//! Groups (in order of "how often does this run on a busy server"):
//!
//! - `envelope` — RequestEnvelope / ResponseEnvelope msgpack encode + decode
//!   (every post-handshake request).
//! - `dispatch` — full server-side per-request path: parse → DashMap lookup →
//!   §7.5 validity check → handler → response encode.
//! - `session_store` — DashMap insert / lookup / remove (sub-µs target).
//! - `crypto_primitives` — HMAC-SHA256, SHA-256, HKDF-SHA256, AES-256-GCM
//!   encrypt+decrypt, Ed25519 sign+verify.
//! - `protocol_construction` — auth_message, identity_input, fake_blob,
//!   ticket encrypt+decrypt (per-handshake / per-resume).
//! - `handshake_verify` — full ServerHandshake::verify_proof for known and
//!   unknown user (~once per connection but sets P99 of the auth pipeline).
//!
//! Wire framing benchmarks live in `shamir-transport-tcp/benches/framing.rs`.
//!
//! Argon2id is intentionally NOT benchmarked — it's a deliberately slow
//! primitive (~2 s with default 128 MB params) and re-measuring it just
//! re-confirms the spec choice.
//!
//! Migrated to the fixed-iteration harness (`bench_scale_tool`): sync
//! workloads (`envelope`, `session_store`, `crypto_primitives`,
//! `protocol_construction`, `scram_post_argon`) use `bench`/`bench_batched`
//! exactly as under Criterion's `b.iter`. The `dispatch` and
//! `handshake_verify` groups drive `async fn`s and now use
//! `bench_async`/`bench_batched_async`, which run on the harness's single
//! shared multi-thread `tokio::Runtime` instead of the per-group
//! current-thread runtime this file used to build by hand.
//!
//! `bench_argon2_derive` (previously part of the Criterion group list) is
//! dropped: per the doc comment above, Argon2id is intentionally not
//! benchmarked, and the old fn was dead code never wired into
//! `criterion_group!`.

use std::hint::black_box;

use bench_scale_tool::Harness;

use shamir_connect::common::auth_message::{AuthMessage, AuthMessageInputs};
use shamir_connect::common::crypto::{
    aes256gcm_cipher, aes256gcm_decrypt, aes256gcm_decrypt_with_cipher, aes256gcm_encrypt,
    aes256gcm_encrypt_with_cipher, constant_time_eq, ed25519_verify_strict, hkdf_sha256,
    hmac_sha256, random_array, sha256, Ed25519Keypair, StoredKey,
};
use shamir_connect::common::envelope::{
    RequestEnvelope, RequestEnvelopeRef, RequestEnvelopeView, ResponseEnvelope,
};
use shamir_connect::common::fake_blob::FakeBlob;
use shamir_connect::common::identity::build_identity_input;
use shamir_connect::common::kdf_params::KdfParams;
use shamir_connect::common::scram::DerivedKeys;
use shamir_connect::common::time::UnixNanos;
use shamir_connect::common::types::{BindingMode, ProtocolVersion, TransportKind};
use shamir_connect::common::username::NormalizedUsername;
use shamir_connect::server::config::{ListenerPolicy, ServerSecrets};
use shamir_connect::server::conn_services::ConnectionServices;
use shamir_connect::server::dispatch::{
    dispatch_request, dispatch_request_view, DispatchOutcome, HandlerFuture, RequestHandler,
};
use shamir_connect::server::handshake::{
    AuthInitView, ProofOutcome, ServerHandshake, SESSION_MAX_AGE_NS,
};
use shamir_connect::server::session::{Session, SessionPermissions, SessionStore};
use shamir_connect::server::ticket::{decrypt_ticket, encrypt_ticket, TicketPlain, TicketWire};
use shamir_connect::server::ticket::{decrypt_ticket_with_ciphers, encrypt_ticket_with_cipher};
// (decrypt_ticket_with_ciphers / encrypt_ticket_with_cipher imported above)
use shamir_connect::server::user_record::UserRecord;

use zeroize::Zeroizing;

// ----------------------------------------------------------------------------
// Shared fixtures (built once outside the benchmarked region).
// ----------------------------------------------------------------------------

/// Faster-than-spec Argon2id params for *fixture construction only* — keeps
/// bench startup short. The benchmarks under test do NOT include Argon2id,
/// so this only affects the one-time setup cost.
fn fast_kdf() -> KdfParams {
    KdfParams {
        memory_kb: 19_456,
        time: 2,
        parallelism: 1,
        argon2_version: 0x13,
    }
}

fn make_auth_message() -> AuthMessage {
    let username = NormalizedUsername::from_raw("alice").unwrap();
    AuthMessage::build(AuthMessageInputs {
        username: &username,
        client_nonce: &[0xaa; 32],
        server_nonce: &[0xbb; 32],
        salt: &[0xcc; 16],
        kdf_params: KdfParams::DEFAULT,
        transport_kind: TransportKind::Tcp,
        binding_mode: BindingMode::TlsExporter,
        tls_exporter_or_zeros: &[0x77; 32],
        supported_version: ProtocolVersion::V1,
    })
    .unwrap()
}

fn make_user_record(password: &[u8]) -> UserRecord {
    let salt = [0x42u8; 16];
    let kdf = fast_kdf();
    let derived = DerivedKeys::derive(password, &salt, &kdf).unwrap();
    let mut sk = Zeroizing::new([0u8; 32]);
    sk.copy_from_slice(&derived.server_key[..]);
    UserRecord {
        salt,
        stored_key: derived.stored_key,
        server_key: sk,
        kdf_params: kdf,
        tickets_invalid_before_ns: 0,
    }
}

fn fixture_session(uid: [u8; 16]) -> Session {
    Session::new(
        uid,
        "alice".into(),
        SessionPermissions::from_roles(vec!["read_write".into()]),
        TransportKind::Tcp,
        BindingMode::TlsExporter,
        [0x77u8; 32],
        UnixNanos::now().as_u64(),
    )
}

struct EchoHandler;

impl RequestHandler for EchoHandler {
    fn handle<'a>(
        &'a self,
        _: &'a Session,
        req: &'a [u8],
        _conn: &'a ConnectionServices,
    ) -> HandlerFuture<'a> {
        let out = req.to_vec();
        Box::pin(async move { Ok(out) })
    }
}

fn main() {
    let mut h = Harness::new("hot_paths", env!("CARGO_MANIFEST_DIR"));

    // ------------------------------------------------------------------
    // Group: envelope (msgpack encode/decode)
    // ------------------------------------------------------------------
    {
        let sid = [0xa1u8; 32];
        // Single small body size — the harness (bench-scale-tool) owns
        // iteration count, so each registered call must be a cheap unit.
        {
            let body_size = 16usize;
            let body = vec![0u8; body_size];
            let env = RequestEnvelope::new(sid, Some(42), body);
            let encoded = env.to_msgpack().unwrap();

            {
                let env = env.clone();
                h.bench(&format!("envelope/request_encode/{body_size}"), move || {
                    let bytes = env.to_msgpack().unwrap();
                    black_box(bytes);
                });
            }

            // Optim #9: borrowed encode path — saves the per-call sid Vec<u8>
            // allocation that `RequestEnvelope::new` does.
            {
                let body_buf = vec![0u8; body_size];
                h.bench(
                    &format!("envelope/request_encode_ref/{body_size}"),
                    move || {
                        let r = RequestEnvelopeRef {
                            session_id: &sid,
                            request_id: Some(42),
                            req: &body_buf,
                        };
                        let bytes = r.to_msgpack().unwrap();
                        black_box(bytes);
                    },
                );
            }

            {
                let encoded = encoded.clone();
                h.bench(&format!("envelope/request_decode/{body_size}"), move || {
                    let parsed = RequestEnvelope::from_msgpack(&encoded).unwrap();
                    black_box(parsed);
                });
            }

            let resp = ResponseEnvelope::ok(Some(42), vec![0u8; body_size]);
            h.bench(
                &format!("envelope/response_encode/{body_size}"),
                move || {
                    let bytes = resp.to_msgpack().unwrap();
                    black_box(bytes);
                },
            );
        }
    }

    // ------------------------------------------------------------------
    // Group: dispatch (full per-request server path)
    //
    // Drives `async fn`s — uses the harness's single shared multi-thread
    // runtime instead of the per-group current-thread runtime the
    // Criterion version built by hand.
    // ------------------------------------------------------------------
    {
        let store = std::sync::Arc::new(SessionStore::new());
        let sid = [0xa1u8; 32];
        let uid = [0x01u8; 16];
        store.insert(sid, fixture_session(uid));
        let handler = std::sync::Arc::new(EchoHandler);

        {
            let body_size = 16usize;
            let env = RequestEnvelope::new(sid, Some(7), vec![0xcdu8; body_size]);
            // Pre-encoded msgpack bytes for the view path (simulates a wire
            // buffer freshly read by the framing layer).
            let env_bytes = env.to_msgpack().unwrap();

            {
                let store = store.clone();
                let handler = handler.clone();
                let env = env.clone();
                h.bench_async(&format!("dispatch/happy_path/{body_size}"), move || {
                    let store = store.clone();
                    let handler = handler.clone();
                    let env = env.clone();
                    async move {
                        let outcome = dispatch_request(
                            &env,
                            &store,
                            |_| 0u64,
                            &*handler,
                            &ConnectionServices::without_push(0),
                        )
                        .await
                        .unwrap();
                        let DispatchOutcome::Response(_) = outcome else {
                            panic!("expected response")
                        };
                    }
                });
            }

            // Optim #4 — borrowed view path. Includes the msgpack PARSE step
            // (which the owning path also does inside `from_msgpack`); the
            // saved work is the per-field Vec<u8> allocations for sid + req.
            {
                let store = store.clone();
                let handler = handler.clone();
                let env_bytes = env_bytes.clone();
                h.bench_async(
                    &format!("dispatch/happy_path_view/{body_size}"),
                    move || {
                        let store = store.clone();
                        let handler = handler.clone();
                        let env_bytes = env_bytes.clone();
                        async move {
                            let view =
                                RequestEnvelopeView::from_msgpack(black_box(&env_bytes)).unwrap();
                            let outcome = dispatch_request_view(
                                &view,
                                &store,
                                |_| 0u64,
                                &*handler,
                                &ConnectionServices::without_push(0),
                            )
                            .await
                            .unwrap();
                            let DispatchOutcome::Response(_) = outcome else {
                                panic!("expected response")
                            };
                        }
                    },
                );
            }
        }
    }

    // ------------------------------------------------------------------
    // Group: session_store (raw DashMap operations)
    // ------------------------------------------------------------------
    {
        let store = SessionStore::new();
        let sid = [0xa1u8; 32];
        let uid = [0x01u8; 16];
        store.insert(sid, fixture_session(uid));

        h.bench("session_store/lookup_hit", move || {
            let s = store.lookup(black_box(&sid));
            black_box(s);
        });
    }
    {
        let store = SessionStore::new();
        let sid = [0xa1u8; 32];
        let uid = [0x01u8; 16];
        store.insert(sid, fixture_session(uid));
        // Optim #5 — lookup_at with caller-supplied timestamp (skips
        // UnixNanos::now() syscall inside the hot path).
        let captured_now = UnixNanos::now().as_u64();
        h.bench("session_store/lookup_at_hit", move || {
            let s = store.lookup_at(black_box(&sid), captured_now);
            black_box(s);
        });
    }
    {
        let store = SessionStore::new();
        let sid = [0xa1u8; 32];
        let uid = [0x01u8; 16];
        store.insert(sid, fixture_session(uid));
        let miss_sid = [0xffu8; 32];
        h.bench("session_store/lookup_miss", move || {
            let s = store.lookup(black_box(&miss_sid));
            black_box(s);
        });
    }
    {
        let store = SessionStore::new();
        let sid = [0xa1u8; 32];
        let uid = [0x01u8; 16];
        store.insert(sid, fixture_session(uid));
        // §7.5 validity check itself is a pure atomic compare; bench in
        // isolation.
        let session_arc = store.lookup(&sid).unwrap();
        h.bench("session_store/validity_check_7_5", move || {
            let ok = session_arc.is_valid_for_user(black_box(0u64));
            black_box(ok);
        });
    }
    {
        let store = SessionStore::new();
        let sid = [0xa1u8; 32];
        let uid = [0x01u8; 16];
        store.insert(sid, fixture_session(uid));
        // Per-session HMAC key derivation. `Session::hmac_key()` is called
        // by the server's destructive-op gate on every drop/clear op in a
        // batch. The cached version returns from an OnceLock after the
        // first call.
        let session_arc_for_hmac = store.lookup(&sid).unwrap();
        h.bench("session_store/hmac_key", move || {
            let k = session_arc_for_hmac.hmac_key();
            black_box(k);
        });
    }

    // ------------------------------------------------------------------
    // Group: crypto_primitives
    // ------------------------------------------------------------------

    // SHA-256
    {
        let size = 32usize;
        let data = vec![0xabu8; size];
        h.bench(&format!("crypto_primitives/sha256/{size}"), move || {
            let h = sha256(black_box(&data));
            black_box(h);
        });
    }

    // HMAC-SHA256 — re-creating the Mac per call (current API).
    {
        let key = [0x55u8; 32];
        let data = [0xabu8; 256];
        h.bench("crypto_primitives/hmac_sha256_256b", move || {
            let tag = hmac_sha256(black_box(&key), black_box(&data));
            black_box(tag);
        });
    }

    // HKDF-SHA256 80 bytes (size used by FakeBlob).
    {
        let ikm = [0x66u8; 32];
        let salt = [0x77u8; 19];
        let info: &[u8] = b"alice";
        h.bench("crypto_primitives/hkdf_sha256_80b", move || {
            let mut out = [0u8; 80];
            hkdf_sha256(black_box(&ikm), black_box(&salt), black_box(info), &mut out).unwrap();
            black_box(&out);
        });
    }

    // AES-256-GCM encrypt + decrypt round-trip on a typical ticket plaintext.
    let aes_key = [0x88u8; 32];
    let nonce = [0x99u8; 12];
    let pt = vec![0xaau8; 256];
    let aad: &[u8] = b"SHAMIR-TICKET-v1\x01";
    let ct = aes256gcm_encrypt(&aes_key, &nonce, &pt, aad).unwrap();
    {
        let pt = pt.clone();
        h.bench("crypto_primitives/aes256gcm_encrypt_256b", move || {
            let out = aes256gcm_encrypt(black_box(&aes_key), &nonce, &pt, aad).unwrap();
            black_box(out);
        });
    }
    {
        let ct = ct.clone();
        h.bench("crypto_primitives/aes256gcm_decrypt_256b", move || {
            let out = aes256gcm_decrypt(black_box(&aes_key), &nonce, &ct, aad).unwrap();
            black_box(out);
        });
    }

    // Optim #3 — cached cipher avoids per-call key schedule.
    let cached_cipher = aes256gcm_cipher(&aes_key).unwrap();
    {
        let cached_cipher = cached_cipher.clone();
        let pt = pt.clone();
        h.bench(
            "crypto_primitives/aes256gcm_encrypt_256b_cached_cipher",
            move || {
                let out =
                    aes256gcm_encrypt_with_cipher(black_box(&cached_cipher), &nonce, &pt, aad)
                        .unwrap();
                black_box(out);
            },
        );
    }
    {
        let cached_cipher = cached_cipher.clone();
        let ct = ct.clone();
        h.bench(
            "crypto_primitives/aes256gcm_decrypt_256b_cached_cipher",
            move || {
                let out =
                    aes256gcm_decrypt_with_cipher(black_box(&cached_cipher), &nonce, &ct, aad)
                        .unwrap();
                black_box(out);
            },
        );
    }

    // Ed25519 sign + verify.
    let kp = Ed25519Keypair::generate();
    let pk = kp.public_bytes();
    let msg = vec![0u8; 200];
    let sig = kp.sign(&msg);
    {
        let kp = kp.clone();
        let msg = msg.clone();
        h.bench("crypto_primitives/ed25519_sign_200b", move || {
            let s = kp.sign(black_box(&msg));
            black_box(s);
        });
    }
    {
        let msg = msg.clone();
        h.bench("crypto_primitives/ed25519_verify_strict_200b", move || {
            let ok = ed25519_verify_strict(black_box(&pk), black_box(&msg), black_box(&sig));
            black_box(ok);
        });
    }

    // ------------------------------------------------------------------
    // Group: protocol_construction
    // ------------------------------------------------------------------

    // auth_message build — 149 bytes for default params.
    let username = NormalizedUsername::from_raw("alice").unwrap();
    {
        let username = username.clone();
        h.bench("protocol_construction/auth_message_build", move || {
            let am = AuthMessage::build(AuthMessageInputs {
                username: &username,
                client_nonce: black_box(&[0xaa; 32]),
                server_nonce: black_box(&[0xbb; 32]),
                salt: &[0xcc; 16],
                kdf_params: KdfParams::DEFAULT,
                transport_kind: TransportKind::Tcp,
                binding_mode: BindingMode::TlsExporter,
                tls_exporter_or_zeros: &[0x77; 32],
                supported_version: ProtocolVersion::V1,
            })
            .unwrap();
            black_box(am);
        });
    }

    // identity_input build (used per-handshake, contains auth_message).
    {
        let am = make_auth_message();
        let pk = [0u8; 32];
        let session_id = [0u8; 32];
        h.bench("protocol_construction/identity_input_build", move || {
            let bytes = build_identity_input(
                black_box(&pk),
                TransportKind::Tcp,
                BindingMode::TlsExporter,
                &[0u8; 32],
                &am,
                &session_id,
                123_456_789,
            );
            black_box(bytes);
        });
    }

    // FakeBlob derive (per unknown-user handshake, runs HKDF-SHA256 80 B).
    {
        let secret = [0xaau8; 32];
        let username = username.clone();
        h.bench("protocol_construction/fake_blob_derive", move || {
            let blob = FakeBlob::derive(black_box(&secret), &username).unwrap();
            black_box(blob);
        });
    }

    // Ticket encrypt + decrypt.
    let ticket_key = [0xddu8; 32];
    let plain = TicketPlain {
        version: 2,
        user_id: serde_bytes::ByteArray::new([0x01u8; 16]),
        username_nfc: "alice".into(),
        transport_kind_at_auth: 0x01,
        binding_mode_at_auth: 0x01,
        channel_binding_at_auth: serde_bytes::ByteArray::new([0x77u8; 32]),
        ticket_family_id: serde_bytes::ByteArray::new([0x11u8; 16]),
        original_auth_at_ns: 1_000_000,
        expires_at_ns: 2_000_000,
        family_counter: 1,
        identity_key_version: 0,
    };
    let wire = encrypt_ticket(&ticket_key, &plain).unwrap();
    {
        let plain = plain.clone();
        h.bench("protocol_construction/ticket_encrypt", move || {
            let w = encrypt_ticket(&ticket_key, black_box(&plain)).unwrap();
            black_box(w);
        });
    }
    {
        let wire = wire.clone();
        h.bench("protocol_construction/ticket_decrypt", move || {
            let p = decrypt_ticket(&ticket_key, None, black_box(&wire)).unwrap();
            black_box(p);
        });
    }

    // Optim #3 — pre-cached cipher (matches the production hot path pattern:
    // ResumeConfig holds the scheduled cipher across requests).
    let ticket_cipher = aes256gcm_cipher(&ticket_key).unwrap();
    {
        let ticket_cipher = ticket_cipher.clone();
        let plain = plain.clone();
        h.bench(
            "protocol_construction/ticket_encrypt_cached_cipher",
            move || {
                let w = encrypt_ticket_with_cipher(&ticket_cipher, black_box(&plain)).unwrap();
                black_box(w);
            },
        );
    }
    {
        let ticket_cipher = ticket_cipher.clone();
        let wire = wire.clone();
        h.bench(
            "protocol_construction/ticket_decrypt_cached_cipher",
            move || {
                let p =
                    decrypt_ticket_with_ciphers(&ticket_cipher, None, black_box(&wire)).unwrap();
                black_box(p);
            },
        );
    }

    // Wire framing for ticket.
    let wire_bytes = wire.to_bytes();
    {
        let wire = wire.clone();
        h.bench("protocol_construction/ticket_wire_to_bytes", move || {
            let bytes = wire.to_bytes();
            black_box(bytes);
        });
    }
    {
        let wire_bytes = wire_bytes.clone();
        h.bench("protocol_construction/ticket_wire_from_bytes", move || {
            let w = TicketWire::from_bytes(black_box(&wire_bytes)).unwrap();
            black_box(w);
        });
    }

    // ------------------------------------------------------------------
    // Group: handshake_verify (end-to-end server-side per-handshake)
    //
    // Drives `async fn`s (`ServerHandshake::new`, `verify_proof`) — migrated
    // to `bench_async`, running on the harness's shared runtime instead of
    // the hand-built current-thread runtime the Criterion version used.
    // ------------------------------------------------------------------
    {
        let identity = std::sync::Arc::new(Ed25519Keypair::generate());
        let secrets = std::sync::Arc::new(ServerSecrets {
            server_secret: [0xaau8; 32],
            lockout_secret: [0xbbu8; 32],
        });
        let policy = ListenerPolicy::new(BindingMode::TlsExporter);
        let kdf = fast_kdf();
        let username = NormalizedUsername::from_raw("alice").unwrap();
        let password = b"correct horse battery staple";
        let user_record = std::sync::Arc::new(make_user_record(password));

        // Pre-compute a valid client_proof so we measure ONLY server-side work.
        let derived = DerivedKeys::derive(password, &user_record.salt, &kdf).unwrap();
        let server_nonce_for_known = [0xbbu8; 32];
        let am_known = AuthMessage::build(AuthMessageInputs {
            username: &username,
            client_nonce: &[0xaau8; 32],
            server_nonce: &server_nonce_for_known,
            salt: &user_record.salt,
            kdf_params: kdf,
            transport_kind: TransportKind::Tcp,
            binding_mode: BindingMode::TlsExporter,
            tls_exporter_or_zeros: &[0x77u8; 32],
            supported_version: ProtocolVersion::V1,
        })
        .unwrap();
        // client_signature = HMAC(stored_key, am); client_proof = client_key XOR client_signature
        let sig = hmac_sha256(&derived.stored_key.0, am_known.as_bytes());
        let mut client_proof = [0u8; 32];
        for i in 0..32 {
            client_proof[i] = derived.client_key[i] ^ sig[i];
        }

        // The actual server_nonce is randomized by ServerHandshake — we cannot
        // bench `verify_proof` end-to-end with a real proof unless we control
        // the nonce. Instead, we measure `ServerHandshake::new()` (challenge
        // computation incl. fake_blob) and a known-bad proof through verify
        // (same code path, just rejection branch).

        // Bench: ServerHandshake::new + challenge for KNOWN user.
        let auth_init_known = AuthInitView {
            user: username.clone(),
            client_nonce: [0xaau8; 32],
            binding_mode: BindingMode::TlsExporter,
            version: 1,
        };
        {
            let secrets = secrets.clone();
            let user_record = user_record.clone();
            let auth_init_known = auth_init_known.clone();
            h.bench_async("handshake_verify/new_known_user", move || {
                let secrets = secrets.clone();
                let user_record = user_record.clone();
                let auth_init_known = auth_init_known.clone();
                async move {
                    let lookup_known = |_u: &NormalizedUsername| -> Option<UserRecord> {
                        Some((*user_record).clone())
                    };
                    let hs = ServerHandshake::new(
                        policy,
                        TransportKind::Tcp,
                        &secrets,
                        auth_init_known,
                        [0x77u8; 32],
                        kdf,
                        lookup_known,
                    )
                    .unwrap();
                    let _ = black_box(hs.challenge());
                }
            });
        }

        // Bench: ServerHandshake::new + challenge for UNKNOWN user
        // (fake_blob path).
        let bob = NormalizedUsername::from_raw("bob").unwrap();
        let auth_init_unknown = AuthInitView {
            user: bob.clone(),
            client_nonce: [0xaau8; 32],
            binding_mode: BindingMode::TlsExporter,
            version: 1,
        };
        {
            let secrets = secrets.clone();
            let auth_init_unknown = auth_init_unknown.clone();
            h.bench_async(
                "handshake_verify/new_unknown_user_fake_blob_path",
                move || {
                    let secrets = secrets.clone();
                    let auth_init_unknown = auth_init_unknown.clone();
                    async move {
                        let lookup_unknown =
                            |_u: &NormalizedUsername| -> Option<UserRecord> { None };
                        let hs = ServerHandshake::new(
                            policy,
                            TransportKind::Tcp,
                            &secrets,
                            auth_init_unknown,
                            [0x77u8; 32],
                            kdf,
                            lookup_unknown,
                        )
                        .unwrap();
                        let _ = black_box(hs.challenge());
                    }
                },
            );
        }

        // Bench: full verify_proof for KNOWN user — the proof we built above
        // won't match because real handshake samples a fresh server_nonce
        // internally, BUT we measure the rejection-path cost (still HMAC +
        // Ed25519 sign + identity_input build, which is what the
        // constant-time discipline ensures we always pay).
        {
            let secrets = secrets.clone();
            let user_record = user_record.clone();
            let identity = identity.clone();
            let auth_init_known = auth_init_known.clone();
            h.bench_async("handshake_verify/verify_proof_rejection_cost", move || {
                let secrets = secrets.clone();
                let user_record = user_record.clone();
                let identity = identity.clone();
                let auth_init_known = auth_init_known.clone();
                async move {
                    let lookup_known = |_u: &NormalizedUsername| -> Option<UserRecord> {
                        Some((*user_record).clone())
                    };
                    let hs = ServerHandshake::new(
                        policy,
                        TransportKind::Tcp,
                        &secrets,
                        auth_init_known,
                        [0x77u8; 32],
                        kdf,
                        lookup_known,
                    )
                    .unwrap();
                    let outcome = hs
                        .verify_proof(&client_proof, &identity, SESSION_MAX_AGE_NS)
                        .unwrap();
                    black_box(outcome);
                }
            });
        }

        // Same but for unknown user (always rejects) — bounds the fake_blob
        // path.
        {
            let secrets = secrets.clone();
            let identity = identity.clone();
            let auth_init_unknown = auth_init_unknown.clone();
            h.bench_async("handshake_verify/verify_proof_unknown_user", move || {
                let secrets = secrets.clone();
                let identity = identity.clone();
                let auth_init_unknown = auth_init_unknown.clone();
                async move {
                    let lookup_unknown = |_u: &NormalizedUsername| -> Option<UserRecord> { None };
                    let hs = ServerHandshake::new(
                        policy,
                        TransportKind::Tcp,
                        &secrets,
                        auth_init_unknown,
                        [0x77u8; 32],
                        kdf,
                        lookup_unknown,
                    )
                    .unwrap();
                    let outcome = hs
                        .verify_proof(&client_proof, &identity, SESSION_MAX_AGE_NS)
                        .unwrap();
                    matches!(outcome, ProofOutcome::Rejected)
                        .then_some(())
                        .expect("expected Rejected");
                }
            });
        }
    }

    // ------------------------------------------------------------------
    // Misc: SCRAM derive (HMAC + SHA without Argon2id) — measures the
    // "after Argon2id finishes" portion of the client side, useful as a
    // baseline for "what does the rest of SCRAM cost beyond the password
    // derivation".
    // ------------------------------------------------------------------
    {
        // Pre-derive once outside the bench; we measure the HMAC+XOR portion.
        let salted = Zeroizing::new([0xaau8; 32]);
        let am = make_auth_message();

        {
            let am = am.clone();
            h.bench("scram_post_argon/hmac_xor_proof_build", move || {
                let stored = StoredKey([0xbbu8; 32]);
                let client_key = Zeroizing::new([0xccu8; 32]);
                let signature = hmac_sha256(&stored.0, am.as_bytes());
                let mut out = [0u8; 32];
                for i in 0..32 {
                    out[i] = client_key[i] ^ signature[i];
                }
                black_box(out);
            });
        }

        // Server: recover client_key + recompute SHA256 + compare.
        {
            let am = am.clone();
            h.bench(
                "scram_post_argon/recover_client_key_and_compare",
                move || {
                    let stored = StoredKey([0xbbu8; 32]);
                    let proof = [0xddu8; 32];
                    let sig = hmac_sha256(&stored.0, am.as_bytes());
                    let mut recovered = [0u8; 32];
                    for i in 0..32 {
                        recovered[i] = proof[i] ^ sig[i];
                    }
                    let recomputed = sha256(&recovered);
                    let ok = constant_time_eq(&recomputed, &stored.0);
                    black_box(ok);
                },
            );
        }

        // Avoid "unused" lint for `salted`.
        let _ = &salted;
        let _ = random_array::<32>();
    }

    h.run();
}
