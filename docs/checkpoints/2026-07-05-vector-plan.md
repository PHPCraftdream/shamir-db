...
origin/master; (3) очистка корня: `opt_crush/`→`docs/audits/`, снёс
...
`docs/roadmap/VECTOR_PRODUCTION_EXECUTION.md` (ревизия по факту кода: поправки
K1–K8, фазы P0–P5+V6, дизайн персистентности, решения) и
`docs/design/where-select-binds.md` (отдельный языковой трек — как отдавать
...
- **`$score` отвергнут** как магичное метаполе. Вместо него — отдельный трек
  «where-бинды» (`docs/design/where-select-binds.md`): `bind` на предикате +
...
- **Делегирование — sh-агенты** (Sonnet top) по prompt-first (бриф в
  `docs/prompts/vector/<NN>-*.md`, коммит ДО запуска).
...
?? docs/checkpoints/2026-07-03-replication-complete.md
