...
| Audit appender (HMAC-chain + audit-line) | `audit_appender.rs` | 492 | 6/6 | ✅ |
...
| MVP E2E test | `tests/mvp_e2e.rs` | ~380 | 1/1 | ✅ TLS + SCRAM + 3 batches roundtrip + resumption ticket |
| Tables registry (durable) | `tables_registry.rs` | ~210 | 4/4 | ✅ wire-table create/drop persists across restart |
| Persistence E2E | `tests/persistence_e2e.rs` | ~330 | 1/1 | ✅ data written via wire survives full restart |
| Framer abstraction (TCP+WS) | `framer.rs` | ~210 | covered by e2e | ✅ generic over Framer trait |
| WS endpoint dispatch | `server.rs` accept loops | ~180 | covered by e2e | ✅ native WSS + browser WSS (Origin allowlist) |
| WS E2E | `tests/mvp_ws_e2e.rs` | ~340 | 1/1 | ✅ wss:// connect → SCRAM → batch through WS frames |
