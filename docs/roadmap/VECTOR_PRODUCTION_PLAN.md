...
(бриф в `docs/prompts/vector/<NN>-<name>.md` коммитится ДО запуска
...
- **1.7. DoD.** Рестарт на 1M векторов ≤ единиц секунд (точный порог
  ...
  отчёт `docs/benchmarks/vector/<date>-persisted-hnsw.md`.
...
- **4.7. DoD.** Bulk-загрузка ≥ 5× быстрее пути по-одному (порог
  уточнить по baseline); отчёт в `docs/benchmarks/vector/`.
...
- **5.4. Доки.** Обновить `docs/guide/06-search.md` (ef_search,
...
- **Prompt-first**: бриф в `docs/prompts/vector/<NN>-<name>.md`,
