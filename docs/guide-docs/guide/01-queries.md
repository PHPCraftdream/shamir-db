...
- **Известное ограничение (#660)**: транзакционный батч, у которого
  ЕДИНСТВЕННЫЙ верхнеуровневый элемент — голый `for_each`/под-батч, СЕГОДНЯ
  не может определить свой repo-скоуп (`distinct_repos`/`table_ref` не
  проходят внутрь тела `Batch`/`ForEach`). Обходной путь — добавить в
  батч ещё хотя бы одну верхнеуровневую операцию (`Read`/`Insert`/и т.п.)
  рядом с `for_each`.
