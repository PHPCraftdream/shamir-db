...
**Где работает.** `$cond`/`$expr`/`$fn`/`$query`/`$param` вычисляются в
`where`-фильтрах, в аргументах `$fn` (`filter.fn(name, args)`), в `when`,
в `for_each`'s `over` — **и, с закрытием #641, также как значение внутри
`INSERT`/`UPDATE`/`SET`(upsert)**: `write.insert(...)`/`.update().set(...)`/
`write.upsert(...)` принимают любой `WriteValue` (Rust: `Doc::set(key, impl
Into<FilterValue>)`), и на вставке/апдейте движок резолвит любой встроенный
маркер `$query`/`$fn`/`$cond`/`$expr` к его РЕАЛЬНОМУ значению — вместо
буквальной записи маркера-объекта как значения поля.

```ts
import { filter, write } from '@shamir/client';

const resp = await db.batch()
  .add('user', db.query('users').where(filter.eq('name', 'alice')))
  .add('order', write.insert('orders', {
    // owner резолвится в РЕАЛЬНОЕ имя первой записи `user` при вставке —
    // не в буквальный объект { "$query": "@user", "path": "[0].name" }.
    owner: filter.queryRef('@user', '[0].name'),
    // band резолвится к "vip"/"regular" по значению total (никакого поля
    // записи не требуется — $cond/$expr внутри write-значения работают
    // только со значениями/queryRef/param, НЕ с $ref на ещё
    // не существующее поле того же документа — см. ограничение ниже).
    band: filter.cond(filter.valueGte(100, 50), 'vip', 'regular'),
    total: 100,
  }))
  .run();
```

**Ограничение (осознанно, не баг): `$ref` внутри write-значения не
поддерживается.** `$ref` = «поле того же документа» — но на момент
резолвинга write-значения самого документа ещё не существует (это как
раз то, что сейчас строится), так что ссылаться на «соседнее поле» через
`$ref` там не на что. `$ref`-узел внутри `$cond`/`$expr`/`$fn` в
write-значении всегда резолвится в «отсутствует» (тот же silent-miss,
что и у любого другого нерезолвящегося значения), а не в ошибку и не в
значение соседнего поля. Используйте `$query`/`$fn`/`$cond`/`$expr` с
литералами/`$query`-рефами/`$param` внутри write-значений; для «поле A +
поле B» вычисляемых при вставке столбцов пока нет прямой поддержки.

Маркер, который не резолвится (например `$query`-реф на несуществующий
алиас, или сам батч ссылается на такой алиас — что также ловится
планировщиком ДО исполнения), — это явная ошибка (`BatchError`,
код `malformed_marker`/`unbound_param` в зависимости от причины), а не
молчаливая запись мусора в таблицу.
...
(`BatchPlanner`) раньше **не рекурсировал** в `$cond`/`$expr`/`$fn` при
извлечении зависимостей между шагами батча — зависимость на
`$query`-реф внутри `$cond`-ветки молча терялась (silent data loss), а
не падала с ошибкой. Это было отслеживается как **#642** и **закрыто**:
`BatchPlanner` теперь рекурсирует в `$cond`/`$expr`/`$fn` для WHERE/`when`
деревьев, и (с #641) — тем же самым разделяемым экстрактором — для
write-значений (`INSERT`/`UPDATE`/`SET`) тоже.
...
`for_each` — data-зависимый цикл внутри батча: `over` резолвится в список
РОВНО ОДИН РАЗ до начала цикла (это может быть `$query`-колонка-реф,
`$fn`-вызов, или литеральный массив), затем тело (`inner`, обычный
под-батч со своим скоупом) исполняется по одному разу на каждый элемент
списка, с текущим элементом, привязанным к именованному параметру
...
в `crates/shamir-client/tests/batch_for_each_e2e.rs`:

```ts
import { write, Query } from '@shamir/client';

const b = db.batch();
b.transactional();

// читаем id всех заказов клиента "alice" — реальный $query
const ordersQ = b.add('orders_q', Query.from('orders').whereEq('customer_id', 'alice'));
const overRef = b.handle('orders_q').column('order_id'); // $query-колонка-реф

// тело цикла: один INSERT в audit_log на каждый order_id, order_id
// приходит из bind_row текущей итерации
const body = db.batch();
body.add('audit', write.insert('audit_log', [
  { order_id: { $param: 'order_id' }, note: 'audited' },
]));

b.forEach('loop', overRef, 'order_id', body);

const resp = await b.run();
resp.results.loop.value; // список per-iteration результатов, по одному на order
```

Ключевые гарантии и ограничения:

- **`over` резолвится один раз, не на каждой итерации.** Если `over` —
  вызов функции с недетерминированным результатом (`now()` и т.п.), он
  вычисляется однократно до старта цикла — список элементов фиксирован
  для всего цикла, а не пересчитывается на каждом шаге.
- **Лимит итераций**: `BatchLimits.max_iterations` (по умолчанию `1000`).
  Для динамического `over` (например `$query`-реф, число элементов
  которого заранее неизвестно) превышение лимита — ошибка, возникающая
  ДО итерации 0 (цикл не запускается частично).
- **Семантика ошибок**: в транзакционном батче падение на любой итерации
  откатывает ВЕСЬ батч (никаких частично применённых итераций). Вне
  транзакции (autocommit) — исполнение останавливается на первой
  упавшей итерации, но уже применённые предыдущие итерации НЕ
  откатываются.
- **Авторизация — пессимистичная.** Тело цикла авторизуется как единый
  шаблон один раз: `for_each` с write-телом требует write-грант
  независимо от реального числа итераций в рантайме (даже если итераций
  окажется 0).
- **Форма результата**: значение алиаса цикла — список per-iteration
  результатов (одна карта на итерацию). Поэлементная адресация вида
  `@loop[i].alias[j].field` сегодня НЕ реализована — сознательно
  отложена (см. ADR, Decision 2); если она вам нужна прямо сейчас, читайте
  `resp.results.loop.value` как обычный массив на стороне клиента.
- **#660 — исправлено**: транзакционный батч, у которого ЕДИНСТВЕННЫЙ
  верхнеуровневый элемент — голый `for_each`/под-батч, теперь корректно
  определяет свой repo-скоуп: `distinct_repos` рекурсивно проходит внутрь
  тел `Batch`/`ForEach` (тело исполняется внутри внешней транзакции, так
  что его repo участвуют и в cross-repo guard). Никаких обходных
  операций-«заглушек» больше не требуется.
- **#661 — исправлено**: тело `for_each`/под-батча (не помеченное
  собственным `transactional`), запущенное внутри уже открытой внешней
  транзакции, теперь ГЕНУИННО участвует в этой транзакции — записи каждой
  итерации/под-батча проводятся через тот же `TxContext`, а не коммитятся
  независимо через implicit per-op транзакцию. До фикса уже успешно
  выполненная запись итерации могла пережить откат внешней транзакции;
  теперь откат внешней транзакции откатывает и её тоже — гарантия
  «падение на любой итерации откатывает ВЕСЬ батч» выше теперь верна не
  только на уровне распространения ошибки, но и на уровне фактической
  durability записанных строк.
- **Производительность (Phase F, честно, не для victory lap)**: `for_each`
  несёт реальный оверхед на итерацию по сравнению с вручную написанным
  плоским батчем — ~1.5–1.6x медленнее для тела из одной операции
  (растёт слегка с ростом N, это не фиксированная стоимость, которая
  амортизируется). Используйте `for_each`, когда N по-настоящему
  data-зависимо/неизвестно на момент написания запроса; если N известно и
  фиксировано заранее, вручную сгенерированный плоский батч будет быстрее.
