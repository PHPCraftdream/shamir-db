...
**Durable по умолчанию — три движка на шкале durability.**

Без поля `engine` (или `engine: 'fjall'`) репозиторий следует дому: если
сервер запущен с `data_dir` (продакшен) — durable (fjall), и
конфигурация таблиц (индексы, валидаторы схемы, буферы), и сами данные
переживают рестарт; если in-memory (тесты) — репозиторий тоже in-memory.

Два явных opt-out по краям шкалы:

**`in_memory`** — эфемерный scratch-пространство: на рестарте пропадает
всё, и конфиг, и данные. Кеши, сессии, временные расчёты:
...
**`hybrid`** — конфигурация таблиц (индексы, валидаторы схемы, буферы,
field-name interner) переживает рестарт; данные — нет. Для таблиц, чья
схема дорога в пересоздании, но сами ряды одноразовые — например,
кеш-таблица со сложным индексом, которая пересеивается из внешнего
источника на каждом старте:
...
`hybrid` требует, чтобы сервер был запущен с `data_dir` (как и durable
`fjall`). Но в отличие от `fjall`/без `engine`, который при отсутствии
`data_dir` тихо падает в `in_memory`, `hybrid` в той же ситуации
ошибается явно — явное обещание сохранить конфиг нельзя тихо понижать
до полного эфемера.
...
* **Durability репозитория = durability дома, плюс явный выбор движка.**
  Без `engine` репозиторий следует дому: с `data_dir` — durable, без —
  in-memory. `in_memory` — всегда эфемерен (всё пропадает на рестарте).
  `hybrid` — конфиг таблиц на диске, данные в памяти (требует `data_dir`,
  иначе явная ошибка, не тихий даунгрейд).
