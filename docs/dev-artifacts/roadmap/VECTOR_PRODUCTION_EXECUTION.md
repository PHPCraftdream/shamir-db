...
3. **`$score` как магичное метаполе — отвергнут.** Отдача ранжирующего числа
   ...
   заметку **`docs/dev-artifacts/design/where-select-binds.md`** (механизм `bind` на
...
- **vector_report** (`examples/vector_report.rs`) — recall@1/@10 (ground truth
  ...
  `docs/dev-artifacts/benchmarks/vector/<date>-*.md`. Recall детерминирован — criterion не нужен.
...
Каждый лист: бриф → `docs/dev-artifacts/prompts/vector/<NN>-<name>.md` (коммит ДО запуска,
...
- **0.4 vector_report + `scripts/bench-vector.sh`** — recall/RSS/build, каталог `docs/dev-artifacts/benchmarks/vector/`.
...
- **6.3 OQL-поверхность + guide** — сверка OQL-форм векторных фич (+ парсер-
  юниты, если есть текстовый синтаксис), `docs/guide-docs/guide/06-search.md`
...
- Фазовые DoD-отчёты в `docs/dev-artifacts/benchmarks/vector/` — воспроизводимы одной командой.
