...
ShamirDB — all-or-nothing видимость (`docs/dev-artifacts/pre-transactional/REVIEW.md:31`,
...
1. **Документированный инвариант (docs-only).** Поднять honest doc-comment
   ...
   и в `docs/dev-artifacts/pre-transactional/05-executor-isolation.md`
...
`docs/dev-artifacts/pre-transactional/REVIEW.md` §11 («Honest follow-ups still OPEN»)
...
Этот документ — часть планируемой серии Phase-доков в `docs/dev-artifacts/roadmap/`.
...
- `docs/dev-artifacts/roadmap/TRANSACTIONS_IMPL.md` — house-style и детальная история
  имплементации tx-слоя.
- `docs/dev-artifacts/roadmap/TRANSACTIONS.md` — дизайн tx и принцип backend-
  agnosticism, на который опирается вердикт MED-A (§1).
- `docs/dev-artifacts/roadmap/PRODUCTION_HARDENING_ROADMAP.md` — тон/структура (honest
  roadmap с оценками + «что НЕ делать»), которой следует этот файл.
- `docs/dev-artifacts/roadmap/PERF_OPPORTUNITIES.md` — куда выносятся perf-остатки
  (per-commit `table_by_token`-cache, batched-flush на redb-стеке).
- `docs/dev-artifacts/pre-transactional/REVIEW.md` — авторитетный §11-список follow-
  ups (см. §4: какие пункты уже CLOSED).
- `docs/dev-artifacts/pre-transactional/05-executor-isolation.md` — "Known Production
