...
Восемь подготовительных этапов в `docs/dev-artifacts/pre-transactional/00-overview.md`.
...
Documented in `docs/dev-artifacts/pre-transactional/05-executor-isolation.md`
"Known Production Limitations" + `docs/dev-artifacts/ops/FLAKY_TESTS.md`. Not bugs —
...
| Documentation files | 12 in `docs/dev-artifacts/pre-transactional/` |
...
- **Атомизация на uniform sub-stages** (each landable, each <1 hour
  ...
  шаге.
- **`docs/dev-artifacts/pre-transactional/*` план up-front.** Каждый этап имел
