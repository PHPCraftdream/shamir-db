...
> **Статус:** ✅ **РЕАЛИЗОВАНА ЦЕЛИКОМ** (②.1a/b/c/**d**, ②.2a/b, ②.3a/b, ②.4a/b/c).
> ②.1d RENAME db — ✅ сделан как чистый каталог-rekey (вариант γ; предпосылка
> отложения оказалась ложной — см. §②.1d). Преемник `NEXT-CAMPAIGN.md` (Phase E) / `PHASE-G-PLAN.md` (Phase G).
> Источник — остаток `docs/dev-artifacts/research/` после кампании ① (Builder parity). Четыре
...
- Стратегия: prompt-first (бриф в `docs/dev-artifacts/prompts/ddl-evolution/` ДО агента, запрет
...
## Итог (реализовано)
✅ Кампания выполнена ЦЕЛИКОМ и запушена: **②.1a/b/c/d** (RENAME folder/group/
role/**db**), **②.2a/b** (FK `ON UPDATE`), **②.3a/b** (unify-uniqueness,
defense-in-depth), **②.4a/b/c** (литерал-`DEFAULT`). Развилки решены: ②.4 — узкий
(B), в составе ②; ②.1d — вариант (γ) каталог-rekey (предпосылка отложения «нужен
fs-move + crash-recovery» оказалась ложной: boot берёт repo-путь из persisted
