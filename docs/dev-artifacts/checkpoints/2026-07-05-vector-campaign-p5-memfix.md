...
`docs/dev-artifacts/roadmap/VECTOR_PRODUCTION_EXECUTION.md`, запущена через /babygoal
(«реализуй всю векторную кампанию»). Конвейер: prompt-first бриф в
`docs/dev-artifacts/prompts/vector/<NN>-*.md` (коммит ДО запуска) → реализация делегатом →
...
(ГРЯЗНОЕ — crush #418 in-flight):
 ...
 M docs/dev-artifacts/benchmarks/vector/2026-07-05-quantization.md
...
`docs/dev-artifacts/prompts/vector/21-free-f32-graph.md`. Далее автономно /crush: #413-415
