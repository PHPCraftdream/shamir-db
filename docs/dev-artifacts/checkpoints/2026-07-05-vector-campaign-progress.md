...
`docs/dev-artifacts/roadmap/VECTOR_PRODUCTION_EXECUTION.md`, запущенной через /babygoal с
целью «реализуй всю векторную кампанию». Конвейер: серийно по одному листу
(таски #393–#415), для каждого — prompt-first бриф в `docs/dev-artifacts/prompts/vector/`
...
?? docs/dev-artifacts/checkpoints/* (несколько, untracked)
```
