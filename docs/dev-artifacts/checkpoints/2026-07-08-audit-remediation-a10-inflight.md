...
- A11-A14 (`docs/dev-artifacts/audits/2026-07-06-concurrency-engine.md`) ещё не начаты: A11 (recovery `wal.commit` без A5-гейта/персиста интернера), A12 (`apply_replicated` без `VersionGuard` клинит watermark), A13 (`remove_table` не чистит `per_table_mvcc`), A14 (конкурентные `drain_all` затирают commit-time ts).
...
?? docs/dev-artifacts/checkpoints/2026-07-08-audit-remediation-a4-inflight.md
