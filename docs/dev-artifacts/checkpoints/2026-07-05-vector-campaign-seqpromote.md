...
РЕАЛИЗАЦИЯ векторной кампании по `docs/dev-artifacts/roadmap/VECTOR_PRODUCTION_EXECUTION.md`
(/babygoal «реализуй всю векторную кампанию»). Конвейер: prompt-first бриф в
`docs/dev-artifacts/prompts/vector/<NN>-*.md` (коммит ДО запуска) → реализация **/crush**
...
верен). Бриф `docs/dev-artifacts/prompts/vector/23-sequential-promote-loss.md` (red-first
