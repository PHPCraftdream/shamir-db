...
Таблица соответствий: `docs/dev-artifacts/prompts/git-history-cleanup/junk-commits-message-map.tsv`
(202 строки, верифицирована — см. `docs/checkpoints`/сессионный лог).
Callback: `docs/dev-artifacts/prompts/git-history-cleanup/commit-callback.py`
...
git filter-repo \
  ...
  --commit-callback "$(cat docs/dev-artifacts/prompts/git-history-cleanup/commit-callback.py)" \
  --force
```
