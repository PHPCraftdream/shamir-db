...
- **F1**: `docs/dev-artifacts/research/coverage-rust-query-builder.md` — «5»/«7» количество →
  привести к одному числу 10 (если ещё не приведено).
- **F2**: `docs/dev-artifacts/research/coverage-ts-tests.md` — it()-счётчики занижены; взять
  актуальные из `npx vitest run` (общее число тестов) и поправить.
- **F3**: `docs/dev-artifacts/research/completeness-oql.md` §1.6 — «12 folders» → 11.
- **F4**: `docs/dev-artifacts/research/completeness-ddl.md` §1.5 — парентеза про SCRAM
  (challenge/response существует) — уточнить формулировку.
- **F5**: `docs/dev-artifacts/research/coverage-ts-query-builder.md` #180 — Rust `one_of`
...
`docs/dev-artifacts/research/*.md` (F1–F5). НЕ трогай Rust-крейты, ddl.ts билдер/тип, write.*,
e2e-файлы.
