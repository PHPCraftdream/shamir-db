...
|| Live subscriptions (server-push) | ❌ Design doc (`docs/roadmap/LIVE_SUBSCRIPTIONS.md`), status: PROPOSED |
|| Chat-протокол | ❌ Нет кода |
|| `shamir-interconnect` crate | ❌ Не существует |

## 2. Планируемая архитектура

Из роадмапа (`docs/roadmap/PLAN.md`, Movement C — "parked by request"):
...
`docs/roadmap/LIVE_SUBSCRIPTIONS.md` (status: PROPOSED).
...
* **Смотри роадмап.** `docs/roadmap/STAGES.md` и `docs/roadmap/PLAN.md`
