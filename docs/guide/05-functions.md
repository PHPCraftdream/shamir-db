...
{
  "results": {
    "fn": { "records": [{ "created_function": "my_fn" }] }
  }
}
```
...
Зарегистрированная функция вызывается через `BatchOp::Call`:
...
Ответ зависит от того, что вернула функция:
...
### Зависимые вызовы (`$query`)

`call` поддерживает `$query`-ссылки, как и другие операции:
...
4. Если валидатор вернул ошибки — запись **отклонена**, в ответе —
  field-bound error codes (`"too_young"`, `"name_required"` и т.п.).
...
```json
{
  "id": "mk-folder",
  "queries": {
    "f": { "create_function_folder": ["reports", "daily"] }
  }
}
```
...
```
create_function  →  invoke (call)  →  rename_function  →  invoke  →  drop_function
create_validator →  bind → (validates inserts) → unbind → drop_validator
```

* `rename_function` / `rename_validator` — атомарное переименование.
* `list_functions` / `list_validators` — интроспекция через `access_tree`.
...
* **`call` — в графе зависимостей.** Можно строить цепочки:
  read → call → read, с `$query`-ссылками между этапами.
