...
открытые access-дефолты. План: `docs/research/PHASE-G-PLAN.md`. Брифы:
`docs/prompts/{query-builder,e2e,access}/*.md`. Стратегия: **вся работа строго
...
**P0 закрыт:** A2 (открытые access-дефолты) реализован Phase G.4 — последний
настоящий невыполненный P0 закрыт. Остаток (B5–B7, E2, E5) — DX/эволюция, в
`ACTION-ITEMS.md`. E1-follow-on (#250) закрыт (см. E.4-followon выше).
