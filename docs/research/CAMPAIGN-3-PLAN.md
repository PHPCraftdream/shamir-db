...
> **Статус:** план сформирован, **ждёт слова о старте**. Преемник
> `DDL-EVOLUTION-PLAN.md` (кампания ②). Источник — пофайловая сверка
> `docs/research/` после кампаний ①/② (и Phase D/E/G): из HIGH-приоритетов
> открытых не осталось; остаток — тест-покрытие, последний engine-фреймворк
> (mutating-валидаторы) и мелкая builder-досборка. Реализация — через `/crush`
> (prompt-first → zero-trust → коммит per-stage); дизайн-проходы — оркестратор сам.
>
