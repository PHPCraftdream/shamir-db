...
## 0. Где мы стоим (после Phase G)

`ACTION-ITEMS.md`: **все P0/P1 закрыты** (Phase D/E/G). Остаток research-корпуса —
только P2/P3 DX-полировка. Фундамент хартии **S·H·A·M·_·R** закрыт; единственный
открытый пилон — **«I» (Interconnected)** = Movement C (`PLAN.md §2`). Нарративная
нить проекта: «как запись пишется/управляется/распространяется» → «как авторы
пишут код внутри БД» → «темпоральность, вложенные tx, duplex, TS-клиент» → «как
**клиенты подписываются на живые данные**» (✅ сделано) → **следующий бит: «как
реплики следуют за лидером»**. Литеральный следующий шаг — **репликация**.
...
### ③ Completeness & DX sweep (P2/P3-остаток)

E5 (unify uniqueness) + TS DX-билдеры (B5 `Doc`, B6 `Handle`, B7 `tryBuild`/
`deliverCall`/interner) + DbRequest-билдеры + e2e-eval добивка (like/regex/$expr/
$cond/aggregateFn/func/history/page/resume).

- **Польза:** ★★☆☆☆ DX-полировка. **Риск:** низкий. **Похожесть:** twin Phase E/G.
- Хорош как **разгрузочная** кампания между крупными, не как сама крупная.
...
**Выход:** `docs/roadmap/REPLICATION.md` (закоммитить ДО кода).
...
в `docs/prompts/replication/` ДО агента; бриф запрещает git-мутации). `REPLICATION.md`
