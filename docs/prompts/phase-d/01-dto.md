...
Goal (Phase D.0): add the `on_delete` referential-action field to the foreign-key DTO + engine mirror + Rust/TS builders + serde tests. NO engine delete-logic in this task (that is D.1). Plan: docs/design/declarative-schema-validators/10-referential-on-delete.md §4.1 / §5(D.0).
