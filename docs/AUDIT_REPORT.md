...
- **`crates/shamir-types/src/codecs/interned/messagepack.rs:111`** — Unbounded recursion decoding untrusted MessagePack — `rmpv::read_value` + `rmpv_value_to_inner` recurse per nesting level with no cap; deep input overflows the stack and aborts the process (remote DoS). The legacy text-encoding path was mitigated by a 128-level guard; the same cap must be added here. — Add a depth cap (e.g. 64/128) on the msgpack parse/conversion.
...
### High
- **`crates/shamir-query-types/src/batch/types.rs:133`** — Unbounded recursion on untrusted nested Filter/FilterValue/Cond → stack-overflow DoS — `BatchOp::deserialize` has no recursion guard and rmp-serde has none; a few thousand nested `$cond`/`not` levels abort the whole server post-handshake. Planner walks recurse again. — Enforce a max nesting depth during deserialization and make the planner walks iterative.
