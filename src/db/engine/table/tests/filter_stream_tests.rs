...
fn parse_filter(json: &str) -> crate::db::query::filter::Filter {
...
async fn extract_names(
    table: &crate::db::engine::table::TableManager,
    results: &[(crate::types::record_id::RecordId, crate::types::value::InnerValue)],
) -> Vec<String> {
    let interner = table.interner().get().await.unwrap();
    let name_key = interner.get_ind("name").unwrap();
    let mut names = Vec::new();
    for (_id, record) in results {
        if let crate::types::value::InnerValue::Map(map) = record {
            if let Some(crate::types::value::InnerValue::Str(name)) = map.get(&name_key) {
...
async fn test_filter_stream_eq_status_active() {
    ...
    let results = table.filter_stream(100, &filter, &ctx).await.unwrap();
...
async fn test_filter_stream_small_batches() {
    ...
    let results = table.filter_stream(2, &filter, &ctx).await.unwrap();
